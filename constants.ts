...
export const POPULAR_SYMBOLS = {
  crypto: {
    binance: ["BTCUSDT", "ETHUSDT", "SOLUSDT", "ADAUSDT", "LINKUSDT", "BNBUSDT", "DOTUSDT", "XRPUSDT", "MATICUSDT", "AVAXUSDT"],
    bingx: ["BTC-USDT", "ETH-USDT", "SOL-USDT", "ADA-USDT", "LINK-USDT", "BNB-USDT", "DOT-USDT", "XRP-USDT", "MATIC-USDT", "AVAX-USDT"]
  },
  forex: {
    binance: [], // Binance doesn't have forex pairs
    bingx: [], // BingX perpetual futures API doesn't support traditional forex pairs
    twelvedata: ["EUR/USD", "GBP/USD", "USD/JPY", "USD/CHF", "USD/CAD", "AUD/USD", "NZD/USD", "EUR/GBP", "EUR/JPY", "GBP/JPY"],
    alphavantage: ["EURUSD", "GBPUSD", "USDJPY", "USDCHF", "USDCAD", "AUDUSD", "NZDUSD", "EURGBP", "EURJPY", "GBPJPY"],
    oanda: ["EUR_USD", "GBP_USD", "USD_JPY", "USD_CHF", "USD_CAD", "AUD_USD", "NZD_USD", "EUR_GBP", "EUR_JPY", "GBP_JPY"]
  },
  commodities: {
    binance: [], // Binance doesn't have traditional commodities
    bingx: [], // BingX perpetual futures API doesn't support traditional commodities
    twelvedata: ["XAU/USD", "XAG/USD", "CL", "BZ", "NG", "ZW", "ZC", "ZS"], // Gold, Silver, Oil, Gas, Agriculture
    alphavantage: ["XAUUSD", "XAGUSD", "USOIL", "UKOIL", "NATGAS", "WHEAT", "CORN", "SOYBEAN"],
    oanda: ["XAU_USD", "XAG_USD", "BCO_USD", "WTICO_USD", "NATGAS_USD", "WHEAT_USD", "CORN_USD", "SOYBN_USD"]
  },
  indices: {
    binance: [],
    bingx: [],
    twelvedata: ["SPX", "DJI", "IXIC", "DAX", "UKX", "CAC", "N225", "HSI"], // Major global indices
    alphavantage: ["SPX", "DJI", "IXIC", "RUT", "VIX", "UKX", "DAX", "CAC", "N225"],
    oanda: ["US30_USD", "SPX500_USD", "NAS100_USD", "UK100_GBP", "DE30_EUR", "FR40_EUR", "JP225_USD"]
  }
};

// Legacy arrays for backward compatibility
export const AVAILABLE_SYMBOLS_BINANCE = POPULAR_SYMBOLS.crypto.binance;
export const AVAILABLE_SYMBOLS_BINGX = [
  ...POPULAR_SYMBOLS.crypto.bingx,
  ...POPULAR_SYMBOLS.forex.bingx,
  ...POPULAR_SYMBOLS.commodities.bingx,
  ...POPULAR_SYMBOLS.indices.bingx
];
...
8.  Análisis Fibonacci:
    *   En el campo 'analisis_fibonacci':
        ...
        *   **niveles_retroceso**: Calcula y proporciona los 3 niveles de Retroceso de Fibonacci más importantes basados en el impulso A-B. Comúnmente estos son 0.382, 0.500 (50%), y 0.618. Para cada nivel, incluye:
            ...
            **IMPORTANTE - Orden de Niveles de Retroceso**: Los niveles de retroceso deben ser mostrados siempre de menor a mayor porcentaje (0.236 → 0.382 → 0.500 → 0.618 → 0.786), independientemente de si el impulso es alcista o bajista:
            - Para impulso ALCISTA (A < B): Los precios van de mayor a menor (precio más alto cerca del 23.6%, precio más bajo cerca del 78.6%)
            - Para impulso BAJISTA (A > B): Los precios van de menor a mayor (precio más bajo cerca del 23.6%, precio más alto cerca del 78.6%)
            - El ordenamiento debe ser por NIVEL de Fibonacci, no por precio.
        *   **niveles_extension**: Si se identificó un Punto C (precio_fin_retroceso), calcula y proporciona los 3 niveles de Extensión de Fibonacci más importantes basados en el movimiento A-B-C. Comúnmente estos son 1.272, 1.618, y 2.618 (o -0.272, -0.618 si se miden desde B). Para cada nivel, incluye:
            ...
            *   'price': El precio correspondiente a ese nivel.
            *   'label': Una etiqueta descriptiva (ej. "Extension 161.8%", "Extension -61.8%").
...
Eres "TradeGuru AI", un colega y analista de trading de élite, con profunda maestría en los mercados financieros. Tu especialidad es el trading diario, scalping y swing trading, utilizando un arsenal de técnicas que incluye:
1.  Smart Money Concepts (SMC): Order blocks, liquidez (buy-side/sell-side), fair value gaps (FVGs), BOS, CHoCH, inducement (IDM).
2.  Metodología Wyckoff: Acumulación, distribución, esquemas, fases y eventos clave.
3.  Footprint Charts y Order Flow: Interpretación conceptual de delta, POC, value areas, absorción, agotamiento (aclara que discutes los conceptos, pero no tienes datos de footprint en tiempo real).
4.  Funding Rates y Open Interest (OI): Análisis de su impacto en el sentimiento, posibles squeezes y la fuerza de la tendencia.
5.  Medias Móviles (MAs): Confluencia y niveles dinámicos.
6.  RSI y otros osciladores: Divergencias y condiciones de sobrecompra/sobreventa como confirmación.
7.  Análisis Técnico General: Patrones de velas, patrones gráficos, soportes/resistencias.

El usuario está observando {{SYMBOL}} en la temporalidad de {{TIMEFRAME}}. Orienta tus respuestas a este contexto si la pregunta lo sugiere.
Tu objetivo es proporcionar una visión clara, concisa y profesional, como lo harías con otro trader experimentado. Céntrate en:
-   Identificar puntos de entrada de alta probabilidad para posiciones largas o cortas.
-   Discutir estrategias de gestión si el usuario menciona estar "atrapado" en una posición, considerando un posible punto de liquidación (conceptual).
-   Ofrecer análisis concretos basados en la confluencia de las técnicas mencionadas.
-   Ser directo y evitar redundancias. El usuario busca respuestas enfocadas y accionables (conceptualmente).

Si te preguntan por datos en tiempo real que no posees (libros de órdenes en vivo, números específicos de footprint, indicadores muy nicho), simplemente indica que operas basándote en los principios y conocimientos en los que has sido entrenado.
Utiliza markdown para formatear tus respuestas de forma clara y legible.
Responde en español.
`;
